...
A while back I built up a circuit intended to prototype a leveling
circuit that could program the output level using a dc control voltage.
Hans Summers showed [such a circuit](https://youtu.be/732ESoul088) for
...
more of the circuit and learn some useful software skills.

# The circuit
This is the circuit, modified only slightly, for LTspice:

![](images/ltspice_schematic.png){width=600px}

I found the IRF510 spice model on the internet.

Steady state analysis of this circuit is deceptively straight forward. 
...
the voltage reference at the positive input of the op-amp. 

# Startup
This circuit does not fit well with an AC analysis in LTspice, since the 
DC operating point at the negative input clearly is 0V. Instead I would like
to simulate a transient analysis. 

## The first 10 ms

Below is the first type of plot of how the circuit starts up I made.

![](images/10us.png){width=600px}

At first, this seemed like a failure. The MOSFET has a volt and a half of 
...
spike. The op-amp cannot deliver more bias, and it just seems non-functional.

## The first 500 us
If we expand the time window to 500 micro seconds, we see more (and violent)
action. 

![](images/500us.png){width=600px}

From this picture it is clear that at initial startup, the circuit is 
...
the drain and also the filtered output. The rectified output reaches almost 12V. 

But then, at about 300us, the amplification starts to turn back, and at 400 us
there is only the DC operating voltage left at the drain since the gate level 
is down to a level where the MOSFET is not turned on.

## The first 200ms

Simulating 500 us of a 10MHz requires a bit of computation time. And
...
the simulations before redrawing, so that is also very slow.

![](images/200ms.png){width=600px}


Here we see that for the first 40 ms, we need to wait for the rectified
...
sense, since the time constant of C10 and R6 is about 7ms. 

When the rectified filtered output reaches the reference level, for a while
...
The circuit keeps its regulated level for the rest of the simulation period.

Would there be advantages to reduce the time constant to reach regulation earlier?
...
current of D2, and this induces a modulation of the output.
